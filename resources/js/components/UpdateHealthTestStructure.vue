      ...
      <div class="breadcrumbbar">
                <div class="row align-items-center">
                    <div class="col-md-8 col-lg-8">
                        <h4 class="page-title">Update Health Test Structure
                    ...
                    <div class="col-md-4 col-lg-4">
                        <div class="breadcrumb-list">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page">Update Health Test Structure</li>
                            </ol>
                        </div>
                    </div>
                </div>
          ...
          <div class="card-body">
            ...
            <div class="card" v-if="$role.isPermission('vehicle_details_list')">
              <div class="card-body table-responsive p-0">
                  ...
                  <div class="row">
                      <input type="hidden" v-bind:value="healthteststructures.id" id="id" name="id">
                     <div class="form-group col-md-6">
                            <label>Name</label> <span class="text-danger">*</span>
                            <input v-bind:value="healthteststructures.name" type="text" name="name"  ref="name" id="name" required 
                         ...
                         <div class="form-group col-md-6">
                            ...
                            <input v-bind:value="healthteststructures.vin_number" type="text" name="vin_number"  ref="vin_number" id="vin_number" required
...
<hr>
                 ...
                 <div class="row">
                  ...
                  <div v-for="(veh,index) in JSON.parse(healthteststructures.field)" :key="veh.id">
                                                                                         
                  <div class="row fieldData" v-bind:id="index+1" style="align-items: center;">
                     <div class="form-group col-md-3">
                           <input type="text" name="category[]"  ref="category"  required  :value="veh.category"
                             
                                class="form-control" :class="{ 'is-invalid': form.errors.has('category') }">
                            <has-error :form="form" field="category"></has-error>
                        </div>
                         <div class="form-group col-md-3">
                           <input type="text" name="field[]"  ref="field"  required  :value="veh.field"
                             
                                class="form-control" :class="{ 'is-invalid': form.errors.has('field') }">
                            <has-error :form="form" field="field"></has-error>
                        </div>
                         <div class="form-group col-md-3">
                           <select  type="text" name="type[]"  ref="type" id="type1" required
                             
                                class="form-control" >
                                <option value="single" v-if="veh.type == 'single'" selected>Single Option</option>
                                <option value="single" v-else>Single Option</option>
                                <option value="multiple" v-if="veh.type == 'multiple'" selected>Multiple Option</option>
                                <option value="multiple" v-else>Multiple Option</option>
                                <option value="input"  v-if="veh.type == 'input'" selected>Input Option</option>
                                <option value="input"  v-else>Input Option</option>
                           </select>
                            
                        </div>
                        <div class="form-group col-md-3">
                           <button class="btn btn-danger btn-sm" type="button" @click="removeRow(index+1)" style="padding: 3px 8px 2px 8px;" title="Remove Row"><i class="fa fa-minus"></i></button>
                        </div>
                  </div>
                   </div>
                  <div id="addRow"></div>
                   ...
                   <button type="submit" class="btn btn-rounded btn-success-rgba" style="padding: 3px 12px 3px 6px;border: 1px solid;"><i class="feather icon-save"></i> Update</button>
                  </form>
              </div>
...
<script>
    export default {
        ...
        methods: {
            addRow(){
             ...
             createHealthTest(){
              ...
              axios.post('/api/updateHealthTestStructure', data)
          ...
          loadUpdateHealthTestStructure(){
              var currentUrl = window.location.pathname;
              var splitUrl = currentUrl.split('/');
              var param = splitUrl[splitUrl.length-1];
              console.log();
                if(this.$gate.isAdmin()){
                    axios.get('/api/updatadatahealthteststructure/'+param).then(({ data }) => (
                        this.healthteststructures = data.data
                        // $('#id').val(data.data.id),
                        // $('#name').val(data.data.name),
                        //  $('#vin_number').val(data.data.vin_number)
                        //  addUpdateRow(data.data.field)
                        ));
                }
            },
        ...
        created() {
            this.loadUpdateHealthTestStructure();
        },
        }
