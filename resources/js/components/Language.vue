      ...
      <div class="breadcrumbbar">
                <div class="row align-items-center">
                    <div class="col-md-8 col-lg-8">
                        <h4 class="page-title">Language
                            <button v-if="$role.isPermission('add_vehicle_type')" type="button" class="btn btn-rounded btn-success-rgba" @click="newModal" style="padding: 3px 12px 3px 6px;border: 1px solid;"><i class="feather icon-plus"></i> Add</button>
    ...
    <div class="contentbar" style="padding:8px 30px 30px 30px !important;">
        <div class="card m-b-30">
          ...
          <div class="card-body">
            ...
            <div class="card" v-if="$role.isPermission('vehicle_type_list')">
              <!-- /.card-header -->
                ...
                <table class="table">
                  <thead>
                     <tr>
                                           <th>S.NO</th>
                                            <th>Name</th>
                                            <th v-if="$role.isAction('update_vehicle_type','delete_vehicle_type')">Action</th>
                                        </tr>
                  </thead>
...
<tr v-for="(language,index) in languages.data" :key="language.id">
                                       ...
                                       <td>{{fromData+index}}</td>
                                        ...
                                        <td v-if="language.id != 1">

                                            <a v-if="$role.isPermission('update_vehicle_type')" href="#" class="btn btn-rounded btn-primary-rgba" @click="editModal(language)">
                                                <i class="fa fa-edit blue"></i>
                                            </a>
                                            &nbsp;
                                            <a v-if="$role.isPermission('delete_vehicle_type')" href="#" class="btn btn-rounded btn-danger-rgba" @click="deleteData(language.id)">
              ...
              <!-- /.card-body -->
              <div class="col-md-12">
                  <pagination :data="languages" @pagination-change-page="getResults"></pagination>
              </div>
            </div>
             <div v-else>
        ...
        <div class="modal fade" id="addNew" tabindex="-1" role="dialog" data-backdrop="static" aria-labelledby="addNew" aria-hidden="true">
            <div class="modal-dialog zoomIn  animated" role="document">
                ...
                <div class="modal-header">
                    <h5 class="modal-title" v-show="!editmode">Add Language</h5>
                    <h5 class="modal-title" v-show="editmode">Update Language</h5>
                ...
                <form ref="addform" @submit.prevent="editmode ? updateLanguage() : createLanguage()">
    ...
    export default {
        ...
        methods: {
            ...
            getResults(page = 1) {
                  ...
                  axios.get('/api/language?page=' + page).then(({ data }) => (this.languages = data.data,this.fromData = data.data.from));
            ...
            checkduplicate(e){
                ...
                this.form.get('/api/languageCheck/'+countriesData+'/'+name)
                .then((response) => {
                ...
                //   text: "You won't be able to revert this!",
                  ...
                  confirmButtonText: 'Yes, delete it!'
                      ...
                      // Send request to the server
                        if (result.value) {
                              this.form.delete('/api/language/'+id).then(()=>{
                                      Swal.fire(
                                      'Deleted!',
            ...
            loadLanguage(){
                if(this.$gate.isAdmin()){
                    axios.get("/api/language").then(({ data }) => (this.languages = data.data,this.fromData = data.data.from));
                }
            },
            createLanguage(){
